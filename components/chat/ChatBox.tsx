...
const ChatBox = ({}: ChatBoxProps) => {
  ...
  return (
    <form className="relative flex flex-row cursor-text gap-2 border-gray-7 transition-colors hover:border-gray-8 flex-wrap rounded-t-2xl border-t px-container py-3 mobile:rounded-3xl mobile:border-2 mobile:px-4">
      <TextareaAutoSize
        ref={textareaRef}
        value={input}
        ...
        placeholder="Messsage Trippy"
        className="scrollbar-hide resize-none bg-transparent outline-none placeholder:text-gray-500 focus:placeholder:text-gray-8 flex-1 text-md mobile:self-center mobile:basis-full"
      />
      {/* Enter button */}
      <button
        name="send message"
        className="absolute top-[4px] right-[6px] flex h-[40px] cursor-pointer items-center rounded-full px-2 hover:rounded-full hover:bg-gray-200 hover:shadow-sm"
        onClick={() => sendMessage()}
      >
        <svg
