...
export const useTripEditorStore = create<TripEditorSate>()((set)=> ({
    ...
    moveActivity: (fromDayIndex: number, toDayIndex: number, fromActivityIndex: number, toActivityIndex: number) =>
      set(produce((state) => {
        ...
        state.tripActivities[toDayIndex].splice(toActivityIndex, 0, movedActivity);
      })),
}))