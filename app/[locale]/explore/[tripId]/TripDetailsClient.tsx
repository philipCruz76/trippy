...
export default function TripDetailsClient({
  ...
  const handlePublish = async () => {
    try {
      ...
      const hasChanges =
        ...
        (coverPhotoCreditName &&
          coverPhotoCreditName !== initialData.photoCreditName) ||
        (coverPhotoCreditLink &&
          coverPhotoCreditLink !== initialData.photoCreditLink) ||
        tripActivities.some((dayActivities, dayIndex) =>
          dayActivities.some((activity, actIndex) => {
            const originalActivity =
              initialData.itinerary.dailyTrip[dayIndex]?.itinerary[0]
                ?.activities[actIndex];
  ...
  useEffect(() => {
    ...
    const updateActiveSection = (scrollPos: number) => {
      ...
      const itineraryTop =
        itineraryRef.current?.getBoundingClientRect().top ?? 0;
      const locationsTop =
        locationsRef.current?.getBoundingClientRect().top ?? 0;
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="left">
                {!isPublishable ? (
                  <p>
                    Please add a title, location and at least one activity to
                    publish your trip
                  </p>
        ...
        <DialogContent className="mobile:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground mt-2">
              Are you sure you want to publish this trip? Published trips will
      ...
      <Dialog open={showUnpublishDialog} onOpenChange={setShowUnpublishDialog}>
        <DialogContent className="mobile:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground mt-2">
              Are you sure you want to unpublish this trip? It will no longer be
